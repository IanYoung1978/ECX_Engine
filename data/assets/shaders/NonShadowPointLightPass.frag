...
// Replaces lightpass.frag's role for point lights with CastsShadow=false - real
// Cook-Torrance/GGX PBR instead of the old Blinn-Phong path, drawn one light per additive
// full-screen-quad pass (matching how PointShadowLightPass.frag's shadow-casting
// counterpart is already invoked), just with no shadow-map sampling - visibility is
// implicitly 1.0. sRGB decode of AlbedoMap is automatic; gamma encode happens once,
// centrally, in hdr_tonemap.frag.
...
void main()
{
    vec4 pcolour = texture(positionMap, indata.VSTexCoord);
    if (pcolour.a == 0.0) discard;
    vec4 ncolour = texture(normalMap, indata.VSTexCoord);
